#!/usr/bin/env python3
"""
Test script to check zoom levels 0.95 and 1.0.
"""
import os
import sys
...
def generate_planet_with_zoom(planet_type="Desert", seed=12345, with_rings=False, zoom_level=None):
    ...
    zoom_text = f"Zoom: {zoom_level:.2f}" if zoom_level is not None else "Default Zoom"
...
def main():
    """
    Generate images for zoom levels 0.95 and 1.0.
    """
    # Create output directory
    output_dir = "output/planets/examples/test_zoom_final_check"
    os.makedirs(output_dir, exist_ok=True)
    
    # Define zoom levels to test
    zoom_levels = [0.95, 1.0]
    
    # Test cases - focus on planets without rings
    test_cases = [
    ...
    for planet_type, seed, with_rings in test_cases:
        ...
        for zoom in zoom_levels:
            ...
            # Save the image
            filename = f"{base_filename}_zoom_{zoom:.2f}.png"
            output_path = os.path.join(output_dir, filename)
            
            planet_image.save(output_path)
            print(f"  - Saved with zoom {zoom:.2f} to {output_path}")
