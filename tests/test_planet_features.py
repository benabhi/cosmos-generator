...
class TestPlanetFeatures:
    ...
    def test_clouds(self, desert_planet, temp_output_dir):
        ...
        desert_planet.cloud_coverage = 0.7
        ...
        assert os.path.exists(os.path.join(cloud_dir, "adjusted_mask.png"))
    
    def test_cloud_coverage(self, desert_planet, temp_output_dir):
        """
        Test cloud coverage parameter.
        """
        # Create a planet with low cloud coverage
        desert_planet.has_clouds = True
        desert_planet.cloud_coverage = 0.2
        
        # Render the planet
        image_low_coverage = desert_planet.render()
        
        # Create a planet with high cloud coverage
        desert_planet.cloud_coverage = 0.8
        
        # Render the planet
        image_high_coverage = desert_planet.render()
        
        # Convert images to arrays for comparison
        array_low_coverage = np.array(image_low_coverage)
        array_high_coverage = np.array(image_high_coverage)
        
        # Check that the images are different
        assert not np.array_equal(array_low_coverage, array_high_coverage)
    ...
    def test_cloud_visibility_enhancement(self, planet_generator, temp_output_dir):
        """
        Test the cloud visibility enhancement feature.
        """
        # Create a planet with clouds
        planet = planet_generator.create("Desert", {
            "size": config.PLANET_SIZE,
            "seed": 12345,
            "clouds": True,
            "cloud_coverage": 1.0,  # Maximum coverage
            "light_angle": 45.0  # Standard light angle
        })
        
        # Render the planet
        image = planet.render()
        
        # Check that the adjusted mask file was created
        cloud_dir = os.path.join(config.PLANETS_CLOUDS_TEXTURES_DIR, "12345")
        adjusted_mask_path = os.path.join(cloud_dir, "adjusted_mask.png")
        assert os.path.exists(adjusted_mask_path)
        
        # Load the adjusted mask
        adjusted_mask = Image.open(adjusted_mask_path)
        
        # Check that the adjusted mask has the correct properties
        assert isinstance(adjusted_mask, Image.Image)
        assert adjusted_mask.mode == "L"  # Grayscale
        assert adjusted_mask.width == config.PLANET_SIZE
        assert adjusted_mask.height == config.PLANET_SIZE
        
        # Check that the mask has non-zero values (clouds are visible)
        mask_array = np.array(adjusted_mask)
        assert np.max(mask_array) > 0
